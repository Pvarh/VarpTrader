...
class CryptoFeed:
    ...
    def get_open_interest(self, symbol: str) -> float | None:
        ...
        Returns the OI in base currency units, or ``None`` on error.
        """
        ...
        try:
            data = self._futures_exchange.fetch_open_interest(perp_symbol)
            oi = float(data.get("openInterestAmount", 0.0))
            logger.info(
                "open_interest_fetched | symbol={} oi={}",
                symbol, oi,
            )
            return oi
        except Exception:
            logger.warning("open_interest_error | symbol={}", symbol)
            return None

    def get_open_interest_history(
        self, symbol: str, timeframe: str = "5m", limit: int = 50,
    ) -> list[dict]:
        """Fetch historical open interest data.

        Returns a list of dicts with 'timestamp' and 'openInterestAmount'.
        """
        if self._futures_exchange is None:
            return []
        ...
        if ":" in symbol:
            ...
            perp_symbol = f"{base}/USDT:USDT"
        try:
            data = self._futures_exchange.fetch_open_interest_history(
            ...
            for entry in data:
                result.append({
                    ...
                    "openInterestAmount": float(entry.get("openInterestAmount", 0)),
                })
            logger.info(
                "open_interest_history_fetched | symbol={} count={}",
                symbol, len(result),
            )
            return result
        except Exception:
            logger.warning(
                "open_interest_history_error | symbol={}",
                symbol,
            )
            return []
    ...
    def get_funding_rate(self, symbol: str) -> float | None:
        ...
        # Translate spot symbol to perp. Binance lists BASE/USDT:USDT only,
        # so BTC/USDC (spot) must map to BTC/USDT:USDT (perp), not BTC/USDC:USDT.
        if ":" in symbol:
            perp_symbol = symbol
        else:
            base = symbol.split("/")[0]
