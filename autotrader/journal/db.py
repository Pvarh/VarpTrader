...
class TradeDatabase:
    ...
    def _migrate(self, conn: sqlite3.Connection) -> None:
        ...
        if "partial_notes" not in columns:
            try:
                conn.execute("ALTER TABLE trades ADD COLUMN partial_notes TEXT")
                conn.commit()
                logger.info("migration_applied | added partial_notes column")
            except Exception:
                pass
    ...
    def update_trade_partial_close(
        ...
        Updates the quantity to the remaining amount and appends a
        partial-fill note to the partial_notes column (keeps
        market_condition clean for regime tagging).
        """
        sql = """
            UPDATE trades
            SET quantity = ?,
                partial_notes = COALESCE(partial_notes, '') ||
                    ' [partial:' || ? || 'qty@' || ? || '=$' || ? || ']'
            WHERE id = ?
        """
        with self._cursor() as cur:
    ...
    def update_trade_context(
        ...
        """Populate regime + swing-bias context on a just-opened trade.

        Only writes columns where the caller supplied a value, so a missing
        swing bias does not overwrite an existing one.
        """
        sets = []
        params: list = []
        if market_condition is not None:
            sets.append("market_condition = ?")
            params.append(market_condition)
        if swing_bias is not None:
            sets.append("swing_bias = ?")
            params.append(swing_bias)
        if swing_confidence is not None:
            sets.append("swing_confidence = ?")
            params.append(int(swing_confidence))
        if not sets:
            return
    ...
    def get_paper_trade_history(self) -> list[dict]:
        """Fetch the full paper-trade journal in insert order."""
        with self._cursor() as cur:
            cur.execute(
                """SELECT * FROM trades
                   WHERE paper_trade = 1
    ...
    def get_recent_closed_trades(
        ...
        *,
        ...
        """Fetch recent closed trades for one symbol/strategy pair."""
        sql = """
            SELECT * FROM trades
            WHERE symbol = ?
              AND strategy = ?
              ...
              AND outcome != 'open'
        """
        params: list[object] = [symbol, strategy]
        if paper_trade_only:
            sql += " AND paper_trade = 1"
        sql += " ORDER BY exit_timestamp DESC, id DESC LIMIT ?"
        params.append(limit)
    ...
    def get_trades_since_by_activity(self, since_iso: str) -> list[dict]:
        """Fetch trades ordered by latest visible activity.

        Uses ``exit_timestamp`` for closed trades when available so dashboards
        show the most recently closed activity first, not just the newest entry.

        Args:
            since_iso: ISO format timestamp string.

        Returns:
            List of trade dictionaries ordered by most recent activity first.
        """
        with self._cursor() as cur:
            cur.execute(
                """SELECT * FROM trades
                   WHERE COALESCE(exit_timestamp, timestamp) >= ?
    ...
    def get_daily_trade_count(self, date_str: str) -> int:
        """Count trades opened on a specific date."""
        with self._cursor() as cur:
            cur.execute(
